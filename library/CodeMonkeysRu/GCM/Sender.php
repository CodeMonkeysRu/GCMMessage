...
class Sender
{
     ...
     */
    public function sendMessage($registrationIds, $data = null, $collapseKey = null)
    {
        $message = new \CodeMonkeysRu\GCM\Message($registrationIds, $data, $collapseKey);
    ...
    private function validatePayloadSize(array $rawData, $fieldName, $maxSize)
    {
        ...
        if (strlen(json_encode($rawData[$fieldName])) > $maxSize) {
            throw new Exception(ucfirst($fieldName) . " payload is to big (max {$maxSize} bytes)", Exception::MALFORMED_REQUEST);
        }
    }
}
